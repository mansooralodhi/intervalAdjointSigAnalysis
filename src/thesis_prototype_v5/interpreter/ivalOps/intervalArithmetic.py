...
class IntervalArithmetic():
    ...
    def maximum(self, a: Union[NDArrayLike, IntervalLike], b: Union[NDArrayLike, IntervalLike]) -> Union[NDArray, Interval]:
        a_is_interval = isinstance(a, tuple)
        b_is_interval = isinstance(b, tuple)

        if a_is_interval and b_is_interval:
            return self.np_like.maximum(a[0], b[0]), self.np_like.maximum(a[1], b[1])

        elif a_is_interval:
            return self.np_like.maximum(a[0], b), self.np_like.maximum(a[1], b)

        elif b_is_interval:
            return self.np_like.maximum(a, b[0]), self.np_like.maximum(a, b[1])

        return self.np_like.maximum(a, b)

    def minimum(self, a: Union[NDArrayLike, IntervalLike], b: Union[NDArrayLike, IntervalLike]) -> Union[NDArray, Interval]:
        a_is_interval = isinstance(a, tuple)
        b_is_interval = isinstance(b, tuple)

        if a_is_interval and b_is_interval:
            return self.np_like.minimum(a[0], b[0]), self.np_like.minimum(a[1], b[1])

        elif a_is_interval:
            return self.np_like.minimum(a[0], b), self.np_like.minimum(a[1], b)

        elif b_is_interval:
            return self.np_like.minimum(a, b[0]), self.np_like.minimum(a, b[1])
    ...
    def equal(self, a: Union[NDArrayLike, IntervalLike], b: Union[NDArrayLike, IntervalLike]) -> Union[NDArray, Interval]:
        a_is_interval = isinstance(a, tuple)
        b_is_interval = isinstance(b, tuple)
    ...
    def divide(self, a: Union[NDArrayLike, IntervalLike], b: Union[NDArrayLike, IntervalLike]) -> Union[NDArray, Interval]:
        if isinstance(b, tuple):
            b = self.np_like.divide(1.0, b[0]), self.np_like.divide(1.0, b[1])
    ...
    def integer_pow(self, a: Union[NDArrayLike, IntervalLike, int], b: Union[int, NDArrayLike, IntervalLike]) -> Union[NDArray, Interval]:
        a_is_interval = isinstance(a, tuple)
        b_is_interval = isinstance(b, tuple)

        if a_is_interval and b_is_interval:
            raise NotImplementedError("Not Implemented, tuple-power-tuple !")

        elif a_is_interval:
            return self.np_like.power(a[0], b), self.np_like.power(a[1], b)

        elif b_is_interval:
            return self.np_like.power(a, b[0]), self.np_like.power(a, b[1])

        return self.np_like.power(a, b)
    ...
    def exp(self, a: Union[NDArrayLike, IntervalLike]) -> Union[NDArray, Interval]:
        if isinstance(a, tuple):
            return self.np_like.exp(a[0]), self.np_like.exp(a[1])
        return self.np_like.exp(a)
    ...
    def squeeze(self, a: Union[NDArrayLike, IntervalLike], axis: int = 0):
        if isinstance(a, tuple):
            return self.np_like.squeeze(a[0], axis), self.np_like.squeeze(a[1], axis)
        return self.np_like.squeeze(a, axis)

    def pad(self, a: Union[NDArrayLike, IntervalLike], pad_width, padding_config):
        if isinstance(a, tuple):
            return self.np_like.pad(a[0], pad_width, padding_config), self.np_like.pad(a[1], pad_width, padding_config)
        return self.np_like.pad(a, pad_width, padding_config)
